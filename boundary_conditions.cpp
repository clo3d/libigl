...
//made a function works for clo avatar(ellie 2023.03.23)
IGL_INLINE bool igl::boundary_conditions2(
	const Eigen::MatrixXd& V,
	const Eigen::MatrixXi& /*Ele*/,
	const Eigen::MatrixXd& C,
	const Eigen::VectorXi& P,
	...
	Eigen::MatrixXd& bc)
{
	...
	vector<int> bci; //will contain vtx index
	vector<int> bcj;  //will contain the colume index (which is same as handle index)
	vector<double> bcv; //will contain the weight value
	...
	for (int p = 0; p < P.size(); p++)
	{
		...
		for (int i = 0; i < V.rows(); i++)
		{
			...
			if (sqrd <= FLOAT_EPS)
			{
				...
				//  pos(1)<<" "<<
				//  pos(2)<<"]).^2) = "<<sqrd<<endl;
				bci.push_back(i); //vtx index
				bcj.push_back(P(p)); //point handle index
				bcv.push_back(1.0); //mapped weight
	...
	for (int e = 0; e < BE.rows(); e++)
	{
		...
		for (int i = 0; i < V.rows(); i++)
		{
			...
			if (t >= -FLOAT_EPS && t <= (1.0f + FLOAT_EPS) && sqrd <= FLOAT_EPS)
			{
				...
				double sqrdtail = (vi - tail).squaredNorm(); //remove index of V when position of V equal to position of tail, Ellie
				if (sqrdtail > FLOAT_EPS)
				{
					bci.push_back(i); //vtx index
					bcj.push_back(BE(e, 0)); //joint index
					bcv.push_back(1.0);
				}
			}
		}
	...
	b.resize(vb.size());
	//std::cout << "bdy condition1 : " << P.size() << std::endl;
	//std::cout << "bdy condition2 : " << BE.rows() << std::endl;
	bc = MatrixXd::Zero(vb.size(), C.rows()); //number of handles = number of joints for joint rigging
	// Map from boundary index to index in boundary
	map<int, int> bim;
	int i = 0;
	// Also fill in b
	...
	Eigen::MatrixXd& bc)
{
	...
	//std::cout << "bdy condition1 : " << P.size() << std::endl;
	//std::cout << "bdy condition2 : " << BE.rows() << std::endl;
	bc = MatrixXd::Zero(vb.size(), numRealHandle); //number of handles = number of joints for joint rigging
	// Map from boundary index to index in boundary
	map<int, int> bim;
	int i = 0;
	// Also fill in b
